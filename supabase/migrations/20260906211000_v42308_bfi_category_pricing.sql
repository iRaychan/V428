-- KeySuite V4.23.08 — BFI pricing-category rule save support.
-- BFI was already supported by the V4.15.11 currency-selection RPC, but the
-- legacy category-rule RPC rejected BFI as an invalid product family.

begin;

create or replace function public.keysuite_save_bfi_category_rule_v42308(
  p_category_id text,
...
begin
  ...
  if v_code<>'BFI' then
    raise exception 'Only BFI is accepted by this pricing-category rule function.';
  end if;

  if coalesce(trim(p_category_id),'')='' then
    raise exception 'Save the Pricing Category first, then set the BFI rule.';
  end if;
  ...
  set category_name=coalesce(nullif(trim(p_category_name),''),category_name),
      product_rules=jsonb_set(coalesce(product_rules,'{}'::jsonb),'{BFI}',v_rule,true)
...
revoke all on function public.keysuite_save_bfi_category_rule_v42308(
  text,text,text,numeric,numeric,numeric,numeric,boolean,boolean,boolean,boolean
) from public,anon;

grant execute on function public.keysuite_save_bfi_category_rule_v42308(
  text,text,text,numeric,numeric,numeric,numeric,boolean,boolean,boolean,boolean
) to authenticated;
