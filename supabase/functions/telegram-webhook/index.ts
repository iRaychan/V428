...
function parseDirectPumpModel(text:any){
  ...
  const es=raw.match(/\bES\s*(\d{1,3})\s*-\s*(\d{1,3}[A-Za-z]?)\b/i);
  if(es)return {family:'ES',model:`ES ${es[1]}-${String(es[2]).toUpperCase()}`};
  return null;
}
...
function smartQuoteRequest(text:any){
  ...
  const motorHpMatch=raw.match(/\b(\d+(?:\.\d+)?)\s*HP\b/i);if(motorHpMatch&&Number(motorHpMatch[1])>0)request.motor_hp=Number(motorHpMatch[1]);
  const seriesMatch=raw.match(/\b(CHC|VMS|SVM|ES)\s*(\d{1,3})(?!\s*-)/i);if(seriesMatch&&!directModel){request.series_filter=`${/^ES$/i.test(seriesMatch[1])?'ES':'CHC'} ${Number(seriesMatch[2])}`;}
  ...
  if(!headMatch&&(request.flow_m3h||directModel?.family==='ES'))headMatch=normalized.match(/\b(\d+(?:\.\d+)?)\s*(mtr|metres?|meters?|m|ft|feet|foot|bar|kpa|psi)\b(?!\s*3)/i);
...
function guidedSelectionPumpLabel(candidate:any){
  ...
  return `${brand} - ${model}${candidate?.undersized===true?' (Undersized)':''}${candidate?.keybot_stock_status==='cold'?' · Cold Item':''}`;
}
...
async function guidedSizeSelectedProducts(service:any,companyId:string,products:any[],q:number,h:number,perProductLimit=3,totalLimit=12,esPole:any=0,seriesFilter:any=''){
  ...
  for(const product of products||[]){
    ...
    for(const pole of poles){let rows:any[]=[];try{rows=selectPumpCandidates(guidedSelectorFamily(group),q,h,pole,Math.max(1,Math.min(240,Number(perProductLimit)||3)),String(seriesFilter||''))||[]}catch(_){rows=[]}
...
function keybotFastPreferDefaultExactBrand(matches:any[],input:any){return Array.isArray(matches)?matches:[]}
...
function customerHintFromMessage(text:any,req:any={}){
  ...
  s=s.replace(/\b(?:CHC|VMS)\s*\d{1,3}\s*-\s*\d{1,3}(?:\s*-\s*\d{1,2})?(?:\s*-\s*\d{1,2})?\b/ig,' ')
    ...
    .replace(/\b\d+(?:\.\d+)?\s*(?:hp|kw)\b/ig,' ')
    .replace(/\b[24]\s*(?:p|pole)s?\b/ig,' ')
    .replace(/\b(?:c\/?w|cw)\b/ig,' ')
  ...
  if(family==='CHC'){
    try{
      ...
      if(user){
        ...
        const hydraulic=await guidedSizeSelectedProducts(service,companyId,products,q,h,240,240,Number(req.es_pole||0),String(req.series_filter||''));
...
async function loadTeskCustomerFeature(service:any,companyId:string,customerId:string){
  ...
  if(r.error){console.warn('[KeySuite V4.28.07] TESK customer feature lookup failed',r.error);return {enabled:false,chemical_selection:false}}
...
function meteringRequestMissing(req:any,_chemicalSelection:boolean){
  // V4.28.07: flow + pressure are the only hard hydraulic requirements.
...
.select('response_mode,keysuite_user_email')
    ...
    // V4.28.07: quick chemical template. Typing chem / chemical (plus common
    ...
    if(!callbackQuery&&text&&!menuText){
      ...
      if(user&&keybotFastExactMayOverrideSession(session)&&keybotFastLooksLikePump(rawTelegramText)&&keybotFastLooksLikeExactPumpModel(rawTelegramText)){
        ...
        const products=scopedProducts.length?scopedProducts:await guidedUserAvailableProducts(service,keySuiteCompanyId,user),matches=keybotFastPrepareMatches(await keybotFastModelMatches(service,keySuiteCompanyId,products,rawTelegramText),rawTelegramText),requestedDuty=keybotFastRequestedDuty(rawTelegramText),requestedPoint=keybotFastRequestedPoint(rawTelegramText),fastContext={keysuite_user_email:user.email,company_curve_only:user?.company_curve_only===true,...(requestedDuty?{keybot_fast_requested_duty:requestedDuty}:{}),...(requestedPoint?{keybot_fast_requested_point:requestedPoint}:{})};if(matches.length===1){session=await keybotFastOpenPreparedMatch(service,telegramToken,keySuiteCompanyId,chatId,senderId,session,null,matches[0],fastContext);return json({ok:true,status:matches[0]?.keybot_fast_choose_pole?'fast_exact_es_pole':'fast_exact_model'})}if(matches.length>1){session=await saveKeybotSession(service,keySuiteCompanyId,chatId,senderId,{mode:'guided',step:'guided_fast_model_choice',selected_customer_id:null,context:{...fastContext,guided_fast_matches:matches}});await telegramSend(telegramToken,chatId,'More than one matching model was found. Choose the desired Brand / Model:',keybotFastMatchKeyboard(matches));return json({ok:true,status:'fast_model_choice',count:matches.length})}await telegramSend(telegramToken,chatId,`No exact assigned pump model matched “${rawTelegramText}”.\n\nTry the exact Brand + Model, for example B.G.Reich ES 80-26 4P.`,isCompanyCurveOnlyUser(user)?companyCurveOnlyMenu():mainMenuMarkup());return json({ok:true,status:'fast_exact_model_not_found'});
      }
...
${keyplcBomText(bom)}`,keyplcManifoldMenu(bom));return json({ok:true,status:'keyplc_manifold_material_updated',material})}
    ...
    // V4.28.07: route TESK Chemical Pump / dosing / metering before normal pump parsing. Chemical-transfer duties remain on the normal pump path.
