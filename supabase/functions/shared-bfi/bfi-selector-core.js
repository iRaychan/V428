...
function evaluateModel(db,m,q,h,hz=50){
  const curve=db?.curves?.[m.series]||db?.curves?.[String(m.series||'').replace(/\D/g,'')];if(!curve)return null;
...
function modelSortParts(model){
  const raw=String(model||'').replace(/^(?:CHC|BFI)\s*/i,'').trim(),nums=raw.split('-').map(Number);
...
g.KeySuiteBFICore=Object.freeze({
